...
%IRSSI = (
    ...
    changed     => $VERSION,
    modules     => 'Data::Dumper XML::RSS LWP::Simple Unicode::String Text::Wrap',
    depends     => 'openurl',
...
use vars qw(@ticker $slide $index $timer_cycle $timer_update %sites $forked);

$index = 0;
# Just to have some data for the first startup
%sites = ( Heise=>{page => 'http://www.heise.de/newsticker/heise.rdf', enable => 1, title=>'', description=>''},
           'Freshmeat'=>{'page' => 'http://freshmeat.net/backend/fm.rdf', 'enable' => 1, title=>'', description=>''}
);
...
sub cmd_newsline ($$$) {
	...
	foreach (@arg) {
	    ...
	    $Text::Wrap::columns = 50; #$witem->window()->{width}/2;
	...
	if (defined($arg[1]) && defined($arg[2])) {
	    my $source = $arg[1];
	    my $page = $arg[2];
	    $sites{$source} = {'page' => $page, 'enable' => "1"};
...
#	}
    ...
    $timer_update = Irssi::timeout_add(Irssi::settings_get_int('newsline_ticker_update_delay'), 'update_ticker', undef) if Irssi::settings_get_int('newsline_ticker_update_delay') > 0;
    Irssi::statusbar_items_redraw('newsline_ticker');
...
sub sig_complete_word ($$$$$) {
    ...
    return unless $linestart =~ /^.newsline (toggle)/;
    foreach (keys %sites) {
	push @$list, $_ if /^($word.*)?$/;
    }
}
...
foreach my $cmd ('description', 'paste', 'paste description', 'fetch', 'reload', 'save', 'list', 'toggle', 'add', 'help') {
...
Irssi::settings_add_int($IRSSI{'name'}, 'newsline_ticker_update_delay', 180000);
