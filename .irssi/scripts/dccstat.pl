# $Id: dccstat.pl,v 1.26 2002/05/14 19:26:30 qvr Exp $
...
%IRSSI = (
    ...
    contributors => "stefan_tomanek\@web.de, dieck\@gmx.de",
    name        => "dccstat",
...
# remember to add dccstat = { }; into your config file
# for example:
#   /statusbar dccstat add dccstat
#   /save
#   statusbar called 'dccstat' will appear automatically when loading the script
#
# default verbose mode will produce output like this 
#       [GET nick: 'foobar.avi' 5500kB of 11MB (50%) 99kB/s ETA: 00:03:00]
# and short mode looks like this:
...
sub dcc_connected {
   ...
   $old=Irssi::settings_get_int('dccstat_refresh');
   Irssi::signal_add_last('dcc destroyed', 'dcc_checklast');
   refresh_dccstat();
}

sub dcc_setupcheck {
   $new=Irssi::settings_get_int('dccstat_refresh');
   if ($new != $old) {
      debug("setting a new refresh-timeout");
      $new = ($new*1000);
      Irssi::timeout_remove($refresh_tag);
      $new = ($dccstat_refresh*1000) if $new < 1000;
      $refresh_tag=Irssi::timeout_add($new, 'refresh_dccstat', undef);
      $old=Irssi::settings_get_int('dccstat_refresh');
   }
   refresh_dccstat();
}
...
sub dcc_calculateETA {
  ...
  $going=1 if $going==0;
  $dccspeed=(($dcc->{transfd}-$dcc->{skipped})/$going);
  ## speed in bytes/sec

  # calculate left transfer size
  # fixed a bug here (which I myself made) -qvr
...
sub dccstat {
  ...
  if ($count>0) {
     ...
     my $getcount=0;
     my (
         $dccpercent,   $dccspeed,      $going,     $dcctype, 
	...
	my $max = Irssi::settings_get_int('dccstat_filename_max_length');
        if (!$mode) { $dccfile=substr($dccfile, 0, $max-2).".." if (length($dccfile) > $max); }
        else {$dccfile=substr($dccfile, 0, 8).".." if (length($dccfile) > 10); }
        ...
        $rot_bar_n = 0 if ($rot_bar_n > scalar(@rot_bar)-1);
        $dccrotbar  = @rot_bar[$rot_bar_n];
        $rot_bar_n++ if (!$get_size_only);
	...
	if (not Irssi::settings_get_bool('dccstat_cycle_through_transfers')) {
	   ...
	   $displaying++ if not $get_size_only;
	}
      } else {
    ...
    sub dcc_fast_refresh {
       ...
       $fref++;
       refresh_dccstat() if Irssi::settings_get_bool('dccstat_EXPERIMENTAL_fast_refresh');
    }
...
Irssi::print("Dccstat.pl loaded - remember to add the dccstat-item to your dccstat-statusbar: /statusbar dccstat add dccstat");
